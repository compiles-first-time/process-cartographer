...
| OQ-09 | Which LOC counting rule do we declare (and hold golden against scc/cloc tolerance)? | no | Builder | U0 | Leaning: physical lines + non-empty count, rule documented in the scorecard; scc/cloc differential lands with the CI oracle harness (U2 gate). Per [ADR-0055](../adr/0055-universal-repo-cartography-computed-not-generated.md). |
| OQ-10 | Do the V8 parse benchmarks (express 97 ms / vue-core 639 ms) hold on WebKit/Safari? | no | Builder | U1 | All first-party numbers are V8 (Node 24 = Chrome engine). Close with a real-browser benchmark incl. one WebKit before claiming cross-browser <5 s at 300k LOC. ADR-0055 consequence. |
| OQ-11 | When exactly does the companion CLI (U5) become necessary, and what are its documented trigger thresholds? | no (blocks U5 only) | Nick + Builder | U5 | Candidate triggers per ADR-0055: GitHub tree-API truncation (>100k files); private repos beyond ~3–5k files / PAT budget; non-Chromium local monorepos; semantic tiers needing native toolchains (Roslyn, PowerShell engine parser). |
