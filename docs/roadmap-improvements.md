...
| A4 ✅ | **Minimap / overview toggle** | Flat whole-city aerial with viewport indicator; needs InstancedMesh at >1k buildings | Drill-down-as-LOD is efficient but loses global orientation. Shipped 2026-07-18 as a 2D-canvas aerial (design change: canvas handles thousands of rects with no InstancedMesh or second WebGL scene): whole-city top-down bottom-right, current district outlined, click-to-jump, collapsible |
...
| A6 ✅ | **Session persistence** | Recent repos + includeDirs choices + annotations in IndexedDB; one-click re-map | Re-ingesting on every visit wastes time and API budget. Shipped 2026-07-18: IndexedDB meta+data stores (cap 5, fail-soft); hero "Recent" list re-maps instantly; restore boundary-validates the stored IR like any IR-JSON load; restored sessions carry no expandDir (disclosed); annotations already persist via the C1 content-hash cache |
...
| B2 ✅ | **Oracle harness in CI** (dependency-cruiser + madge + tsc 2-of-3 for TS; Grimp for Python; pinned-SHA corpus) | "Accurate" becomes a *measured, published* number per release — the 0055 promise operationalized. Shipped 2026-07-18: `npm run oracle` → [docs/accuracy.md](./accuracy.md) + scorecard pill; CI gate `oracle:assert`. Measured: express 100%/100%, vue-core 99.3%/100%, swarm 85.7%/100% (P/R; triage in accuracy.md). First run caught + fixed a real B1 bug (bare-dot imports) — see lessons-learned |
